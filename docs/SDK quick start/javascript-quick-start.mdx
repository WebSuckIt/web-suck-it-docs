...
You can obtain your `USER_ID`, `ACCESS_KEY`, and `PUBLIC_KEY` from the [WebSuckIt dashboard](https://websuckit.com). For guidance on how to do this, please refer to the [quick start guide](https://websuckit.com/docs/quick-start).

## Channel

### Create a channel

To create a channel, you can use the `createChannel` method, it returns a promise that resolves to a channel object. This method takes an object with the `channel` property, specifying the name of the channel you want to create. The channel name should be in the format of a slug, for example, 'test-channel'.
...
You can retrieve a list of all your channels using the `getChannels` method.
...
You can use the `getOrCreateChannel` method to get an existing channel or create a new one.
